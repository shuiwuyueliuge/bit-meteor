...
public class TrackerHttpResponse {
    ...
    public TrackerHttpResponse(byte[] data) throws BencodingParseException {
        List<BencodingData> result = BencodingDecoder.parse(data);
        if (result.size() != 1) {
            return;
        }

        TrackerRespBencodingDataVisitor visitor = new TrackerRespBencodingDataVisitor(this);
        result.get(0).visit(visitor);
//
...
//        }
//
//        BencodingData mItl = bencodingData.getFromDictionary("min interval");
//        if (mItl != null) {
